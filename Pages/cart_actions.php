...
if (!isset($_SESSION['user_id'])) {
    ...
    echo json_encode(['success' => false, 'message' => 'onnect to manage the card.']);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'remove' && isset($_POST['cart_id'])) {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Impossible  to delete the article']);
            exit();
        }
    }
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            if ($updateAction === 'increase') {
                ...
                if ($currentQuantity < $stockQuantity && $currentQuantity < 10) {
                    ...
                    echo json_encode([
                        'success' => false, 
                        'message' => 'Maximum quantity reached or insufficent stock.'
                    ]);
                    exit();
                }
...
header('Content-Type: application/json');
echo json_encode(['success' => false, 'message' => 'Action not valide.']);
exit();